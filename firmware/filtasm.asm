...
;**********************************************************************
        .global _fir2_20    ; declare function as global so c-code can find it
_fir2_20:   ; FIR filter, 2 channels, s1 = 20
;       setc    OVM             ; set overflow mode to hard limit accumulations (for s2=0)
;       spm     0               ; clear product shift mode bits (PM)

; ch a:
        lacl    _in_a           ; in -> ACC
        lar     AR2, _data_ptr_a ; point to state data location d0
        sacl    *,AR0           ; ACC -> d0
        lar     AR0, _data_ptr  ; point to first state data addr used (_data_ptr) -> AR0
        ...
        mac     0ff00h,*-       ; ACC + shifted(P) -> ACC
                                ; d(N-1) -> T
                                ; d(N-1) * coef(N-1) -> P
        rpt     _orderm2        ; i = 2 to N (_orderm2 = N - 2)
        macd    0ff01h, *-      ; ACC + shifted(P) -> ACC
                                ; d(N-i) -> T
                                ; d(N-i) * coef(N-i) -> P
                                ; d(N-i) -> d(N-i+1)
        apac                    ; ACC + shifted(P) -> ACC
        sach    _out_a,2        ; shifted(ACC) -> out (shift by s2)
; Hard limit if there was a positive or negative overflow (only use if s2~=0):
        bcnd    fir2_20neg_a,LT ; branch if ACC<0
        and     #6000h,16   ; check for high order bits set
        bcnd    fir2_20done_a,EQ    ; no pos. overflow if ACC=0
        splk    #7fffh,_out_a ; save most pos. value
        b       fir2_20done_a
fir2_20neg_a:
        and     #6000h,16   ; mask of all but high order bits
        xor     #6000h,16   ; negate high order bits
        bcnd    fir2_20done_a,EQ    ; no neg. overflow if ACC=0
        splk    #8000h,_out_a   ; save most neg. value
fir2_20done_a:
...
; ch b:
        ...
        macd    0ff81h, *-      ; ACC + shifted(P) -> ACC
                                ...
                                ; d(N-i) -> d(N-i+1)
        apac                    ; ACC + shifted(P) -> ACC
        sach    _out_b,2        ; shifted(ACC) -> out (shift by s2)
; Hard limit if there was a positive or negative overflow (only use if s2~=0):
        bcnd    fir2_20neg_b,LT ; branch if ACC<0
        and     #6000h,16   ; check for high order bits set
        bcnd    fir2_20done_b,EQ    ; no pos. overflow if ACC=0
        splk    #7fffh,_out_b ; save most pos. value
        b       fir2_20done_b
fir2_20neg_b:
        and     #6000h,16   ; mask of all but high order bits
        xor     #6000h,16   ; negate high order bits
        bcnd    fir2_20done_b,EQ    ; no neg. overflow if ACC=0
        splk    #8000h,_out_b   ; save most neg. value
fir2_20done_b:
        ret
        
;**********************************************************************
        .global _fir2_21    ; declare function as global so c-code can find it
_fir2_21:   ; FIR filter, 2 channels, s1 = 21
;       setc    OVM             ; set overflow mode to hard limit accumulations (for s2=0)
;       spm     0               ; clear product shift mode bits (PM)
...
; ch a:
        ...
        macd    0ff01h, *-      ; ACC + shifted(P) -> ACC
                                ...
                                ; d(N-i) -> d(N-i+1)
        apac                    ; ACC + shifted(P) -> ACC
        sach    _out_a,1        ; shifted(ACC) -> out (shift by s2)
; Hard limit if there was a positive or negative overflow (only use if s2~=0):
        bcnd    fir2_21neg_a,LT ; branch if ACC<0
        and     #4000h,16   ; check for high order bits set
        bcnd    fir2_21done_a,EQ    ; no pos. overflow if ACC=0
        splk    #7fffh,_out_a ; save most pos. value
        b       fir2_21done_a
fir2_21neg_a:
        and     #4000h,16   ; mask of all but high order bits
        xor     #4000h,16   ; negate high order bits
        bcnd    fir2_21done_a,EQ    ; no neg. overflow if ACC=0
        splk    #8000h,_out_a   ; save most neg. value
fir2_21done_a:
...
; ch b:
        ...
        macd    0ff81h, *-      ; ACC + shifted(P) -> ACC
                                ...
                                ; d(N-i) -> d(N-i+1)
        apac                    ; ACC + shifted(P) -> ACC
        sach    _out_b,1        ; shifted(ACC) -> out (shift by s2)
; Hard limit if there was a positive or negative overflow (only use if s2~=0):
        bcnd    fir2_21neg_b,LT ; branch if ACC<0
        and     #4000h,16   ; check for high order bits set
        bcnd    fir2_21done_b,EQ    ; no pos. overflow if ACC=0
        splk    #7fffh,_out_b ; save most pos. value
        b       fir2_21done_b
fir2_21neg_b:
        and     #4000h,16   ; mask of all but high order bits
        xor     #4000h,16   ; negate high order bits
        bcnd    fir2_21done_b,EQ    ; no neg. overflow if ACC=0
        splk    #8000h,_out_b   ; save most neg. value
fir2_21done_b:
        ret
        
;**********************************************************************
        .global _fir2_22    ; declare function as global so c-code can find it
_fir2_22:   ; FIR filter, 2 channels, s1 = 22
        setc    OVM             ; set overflow mode to hard limit accumulations (for s2=0)
;       spm     0               ; clear product shift mode bits (PM)
...
; Hard limit if there was a positive or negative overflow (only use if s2~=0):
; Fix: consider getting rid of this limiter: it only works if there is no ACC
; overflow, and seting overflow mode only slightly reduces dynamic range in the
; compared to a valid data wraparound.
        bcnd    fir2_15neg_b,LT ; branch if ACC<0
        and     #4000h,16   ; check for high order bits set
        bcnd    fir2_15done_b,EQ    ; no pos. overflow if ACC=0
        splk    #7fffh,_out_b ; save most pos. value
        b       fir2_15done_b
fir2_15neg_b:
        and     #4000h,16   ; mask of all but high order bits
        xor     #4000h,16   ; negate high order bits
        bcnd    fir2_15done_b,EQ    ; no neg. overflow if ACC=0
        splk    #8000h,_out_b   ; save most neg. value
fir2_15done_b:



; Direct memory access SOS's        
;   ldp #_coefdata
;       lt      _d11a       ; d2 -> T
;       mpy     *+      ; T*a2 -> P
;       lta     _d21a       ; d1 -> T, ACC+P -> ACC
;       mpy     *+      ; T*a1 -> P
;       apac                ; ACC+P -> ACC
;       sach    _d01a,1 ; shifted(ACC) -> d0 (shift by s2 = 16-s1)
;       lacl    #0          ; 0 -> ACC
;       mpy     *+      ; T*b2 -> P
;       ltd     _d11a       ; d1 -> T, d1 -> d2, ACC+P -> ACC
;       mpy     *+      ; T*b1 -> P
;       ltd     _d01a       ; d0 -> T, d0 -> d1, ACC+P -> ACC
;       mpy     *+      ; T*b0 -> P
        
;       lta     _d12a       ; d2 -> T, ACC+P -> ACC
;       mpy     *+      ; T*a2 -> P
;       lta     _d22a       ; d1 -> T, ACC+P -> ACC
;       mpy     *+      ; T*a1 -> P
;       apac                ; ACC+P -> ACC
;       sach    _d02a,1 ; shifted(ACC) -> d0 (shift by s2 = 16-s1)
;       lacl    #0          ; 0 -> ACC
;       mpy     *+      ; T*b2 -> P
;       ltd     _d12a       ; d1 -> T, d1 -> d2, ACC+P -> ACC
;       mpy     *+      ; T*b1 -> P
;       ltd     _d02a       ; d0 -> T, d0 -> d1, ACC+P -> ACC
;       mpy     *+      ; T*b0 -> P
;   ldp #temp

;**********************************************************************
; FIR Filter functions (fix: old dump)
; Use different function for different max impulse response value:
;                       fir2_15 - 2 channel, s1=15, s2=0, PM=1
;                       fir2_16 - 2 channel, s1=16, s2=0, PM=0
;
;                       fir1_15 - 1 channel, s1=15, s2=0, PM=1
;                       fir1_16 - 1 channel, s1=16, s2=0, PM=0
;
; C-code sets the following values:
;
;   _func_addr      =   address of function to call
;   _orderm2        =   N - 2, N=2 to 128 for 2 ch, or to 256 for 1 ch filters
;   _data_ptr       =   points to address first used of filter state data (usualy 03ffh)
;   _data_ptr_a =   _data_ptr-N+1, (points to first address filter data ch a: d0)
;   _data_ptr_b =   _data_ptr-2*N+1, (points to first address filter data ch b: d0)
;
;       _data_ptr_b:    d0      - ch b
;                       d1 ...
;                       d(N-1)
...
; Valid combinations (assuming truncation of scaled ideal coefs):
;           max(h)          use:    s1  s2  PM  function
;       [0.5       to 1)            15  0   1   _firX_15
;       [0         to 0.5)          16  0   0   _firX_16
;
;       [0.015625  to 0.03125)      _firX_20 (not used)
;       [0.0078125 to 0.015625)     _firX_21 (not used)
;       [0         to 0.0078125)    _firX_22 (not used)
;                       
;**********************************************************************
        .global _fir2_15    ; declare function as global so c-code can find it
_fir2_15:   ; FIR filter, 2 channels, s1 = 15
        spm     1               ; set product mode (PM) to 1
...
;**********************************************************************
        .global _fir2_16    ; declare function as global so c-code can find it
_fir2_16:   ; FIR filter, 2 channels, s1 = 16
;       spm     0               ; clear product shift mode bits (PM) (this is alread the case)
...
;**********************************************************************
; IIR Filter functions (this is old code):
;                       iir2_4  - 2 channel, 4th order
;                       iir2_8  - 2 channel, 8th order
;                       iir2_12 - 2 channel, 12th order
;                       iir2_16 - 2 channel, 16th order
;                       iir2_20 - 2 channel, 20th order
;                       iir2_24 - 2 channel, 24th order
;
;                       iir1_4  - 1 channel, 4th order
;                       iir1_8  - 1 channel, 8th order
;                       iir1_12 - 1 channel, 12th order
;                       iir1_16 - 1 channel, 16th order
;                       iir1_20 - 1 channel, 20th order
;                       iir1_24 - 1 channel, 24th order
;
;
; C-code sets the following values:
;
;   _func_addr      =   address of function to call
;   _coef_ptr       =   points to first filter coeficient of ch a (in B0: 200h-2ffh):
;
;       _coef_ptr:      a21     - ch a, section 1
;                       a11
;                       b21
;                       b11
;                       b01
...
;                       ... same as ch a above
;
; Coeficent values are stored = int[(2^s1)*true_coef_value]
;
;   _data_ptr       =   points to first used address of filter state data (in B1: 300h-3ffh)
;
;                       d01     - ch b, last section
;                       ...
;                       d02
;                       d12
;                       d22     - ch a, section 2
;                       d01
;                       d11
;       _data_ptr:      d21     - ch a, section 1

;**********************************************************************
        .global _iir2_4 ; declare function as global so c-code can find it
_iir2_4:    ; 2 channel, forth order filter (2 SOS's):
;   setc    OVM             ; set overflow mode to hard limit accumulations
        mar     *,AR0           ; AR0 -> ARP
        lar     AR0,  #03ffh  ; point to first state data addr -> AR0
        
; ch a:
        lacc    _in_a,15    ; shifted(in) -> ACC (shift by s1 = 16-s2)
        lt      *-,AR2      ; d2 -> T
        mpy     *+,AR0      ; T*a2 -> P
        lta     *+,AR2      ; d1 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*a1 -> P
        lta *-              ; d2 -> T, ACC+P -> ACC
        mar *-              ; AR0-1 -> AR0 (point to d0)
        sach    *+,1,AR2    ; shifted(ACC) -> d0 (shift by s2 = 16-s1)
        lacl    #0          ; 0 -> ACC
        mpy     *+,AR0      ; T*b2 -> P
        ltd     *-,AR2      ; d1 -> T, d1 -> d2, ACC+P -> ACC
        mpy     *+,AR0      ; T*b1 -> P
        ltd     *-,AR2      ; d0 -> T, d0 -> d1, ACC+P -> ACC
        mpy     *+,AR0      ; T*b0 -> P

        lta     *-,AR2      ; d2 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*a2 -> P
        lta     *+,AR2      ; d1 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*a1 -> P
        lta *-              ; d2 -> T, ACC+P -> ACC
        ...
        apac
        sach    _out_a,1    ; shifted(ACC) -> out (shift by s2 = 16-s1)

    b   iir2_4done_a
; Hard limit if there was a positive or negative overflow (only use if s2 != 0):
        bcnd    iir2_4neg_a,LT  ; branch if ACC<0
        and     #4000h,16   ; check for high order bits set
        bcnd    iir2_4done_a,EQ ; no pos. overflow if ACC=0
        splk    #7fffh,_out_a ; save most pos. value
        b       iir2_4done_a
iir2_4neg_a:
        and     #4000h,16   ; mask of all but high order bits
        xor     #4000h,16   ; negate high order bits
        bcnd    iir2_4done_a,EQ ; no neg. overflow if ACC=0
        splk    #8000h,_out_a   ; save most neg. value
iir2_4done_a:

; ch b:
        lacc    _in_b,15    ; shifted(in) -> ACC (shift by s1 = 16-s2)
        lt      *-,AR2      ; d2 -> T
        mpy     *+,AR0      ; T*a2 -> P
        lta     *+,AR2      ; d1 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*a1 -> P
        ...
        apac
        sach    _out_b,1    ; shifted(ACC) -> out (shift by s2 = 16-s1)

    b   iir2_4done_b
; Hard limit if there was a positive or negative overflow (only use if s2 != 0):
        bcnd    iir2_4neg_b,LT  ; branch if ACC<0
        and     #4000h,16   ; check for high order bits set
        bcnd    iir2_4done_b,EQ ; no pos. overflow if ACC=0
        splk    #7fffh,_out_b ; save most pos. value
        b       iir2_4done_b
iir2_4neg_b:
        and     #4000h,16   ; mask of all but high order bits
        xor     #4000h,16   ; negate high order bits
        bcnd    iir2_4done_b,EQ ; no neg. overflow if ACC=0
        splk    #8000h,_out_b   ; save most neg. value
iir2_4done_b:
...
;**********************************************************************
        .global _iir1_4 ; declare function as global so c-code can find it
_iir1_4:    ; 1 channel, forth order filter (2 SOS's):
;   setc    OVM             ; set overflow mode to hard limit accumulations
        mar     *,AR0           ; AR0 -> ARP
        lar     AR0, #03ffh  ; point to first state data addr used -> AR0
...
; Hard limit if there was a positive or negative overflow (only use if s2 != 0):
        bcnd    iir1_4neg,LT    ; branch if ACC<0
        and     #4000h,16   ; check for high order bits set
        bcnd    iir1_4done,EQ   ; no pos. overflow if ACC=0
        splk    #7fffh,_out_a ; save most pos. value
        b       iir1_4done
iir1_4neg:
        and     #4000h,16   ; mask of all but high order bits
        xor     #4000h,16   ; negate high order bits
        bcnd    iir1_4done,EQ   ; no neg. overflow if ACC=0
        splk    #8000h,_out_a   ; save most neg. value
iir1_4done:
        ret
...
;**********************************************************************
; IIR Filter functions (this is the latest IIR code):
;                       iir_4_a     - Channel A, 4th order
;                       iir_4_b     - Channel B, 4th order
;                       iir_8_a     - Channel A, 8th order
;                       iir_8_b     - Channel B, 8th order
;                       iir_12_a    - Channel A, 12th order
;                       iir_12_b    - Channel B, 12th order
;                       iir_16_a    - Channel A, 16th order
;                       iir_16_b    - Channel B, 16th order
;                       iir_20_a    - Channel A, 20th order
;                       iir_20_b    - Channel B, 20th order
;                       iir_24_a    - Channel A, 24th order
;                       iir_24_b    - Channel B, 24th order
;
; C-code sets the following values:
;
;   _func_addr_a    =   address of Ch A function
...
;   _data_ptr_a     =   points to first used state address Ch A (in B1: 3ffh)
;
; 300h  _coef_ptr_b:    a21     - Ch B, section 1
;                       a11
;                       b21
;                       b11
;                       b01
...
;                       b12
;                       b02
;                       ... more sections (coef and data grow towards each other)
;
;                       ...
;                       d02
;                       d12
...
;                       d01
;                       d11
; 37fh  _data_ptr_b:    d21     - Ch B, section 1
;
;
; 380h  _coef_ptr_a:    a21     - Ch A, section 1
;                       a11
;                       b21
;                       b11
;                       b01
...
;                       d01
;                       d11
; 3ffh  _data_ptr_a:    d21     - Ch A, section 1
;
;
; Coeficent values are stored = int[(2^s1)*true_coef_value]
;                               with: s1 = 14  => int[16384*true_coef_value]
;
;**********************************************************************
        .global _iir_4_a    ; declare function as global so c-code can find it
_iir_4_a:   ; Ch A, Forth order filter (2 SOS's):
        lacc    _in_a,14    ; shifted(Ch A in) -> ACC (shift by s1 = 16-s2)

        spm     0           ; clear product mode (PM) to 0
        lar     AR2, _coef_ptr_a ; point to first coef address
        lar     AR0, _data_ptr_a ; point to state data location d0
        mar     *,AR0       ; AR0 -> ARP
        
        lt      *-,AR2      ; d2 -> T
        mpy     *+,AR0      ; T*a2 -> P
        lts     *+,AR2      ; d1 -> T, ACC-P -> ACC
        mpy     *+,AR0      ; T*a1 -> P
        lts *-              ; d2 -> T, ACC-P -> ACC
        mar *-              ; AR0-1 -> AR0 (point to d0)
        sach    *+,2,AR2    ; shifted(ACC) -> d0 (shift by s2 = 16-s1)
        lacl    #0          ; 0 -> ACC
        mpy     *+,AR0      ; T*b2 -> P
        ltd     *-,AR2      ; d1 -> T, d1 -> d2, ACC+P -> ACC
        mpy     *+,AR0      ; T*b1 -> P
        ...
        apac
        sach    _out_a,2    ; shifted(ACC) -> out (shift by s2 = 16-s1)
...
;**********************************************************************
        .global _iir_4_b    ; declare function as global so c-code can find it
_iir_4_b:   ; Ch B, Forth order filter (2 SOS's):

; Start of Ch B
        lacc    _in_b,14    ; shifted(Ch B in) -> ACC (shift by s1 = 16-s2)
        bit     _assembly_flag, 13  ; cascade_flag -> TC
        bcnd    iir_4_skip,NTC  ; skip cascade hold if flag not set
        lacc    _out_a,14   ; shifted(Ch A out) -> ACC (shift by s1 = 16-s2)
iir_4_skip:

        spm     0           ; clear product mode (PM) to 0
        lar     AR2, _coef_ptr_b ; point to first coef address
        lar     AR0, _data_ptr_b ; point to state data location d0
        mar     *,AR0       ; AR0 -> ARP
        ...
        apac
        sach    _out_b,2    ; shifted(ACC) -> out (shift by s2 = 16-s1)

        ret
        
 
;**********************************************************************
; Lattice Filter functions:
;                       lattice_2_a     - Channel A, 2nd order
;                       lattice_2_b     - Channel B, 2nd order
;                       lattice_4_a     - Channel A, 4th order
;                       lattice_4_b     - Channel B, 4th order
;                       lattice_8_a     - Channel A, 8th order
;                       lattice_8_b     - Channel B, 8th order
;                       lattice_12_a    - Channel A, 12th order
;                       lattice_12_b    - Channel B, 12th order
;                       lattice_16_a    - Channel A, 16th order
;                       lattice_16_b    - Channel B, 16th order
;
; C-code sets the following values:
;
;   _func_addr_a    =   address of Ch A function
...
;
;
; 300h  _coef_ptr_b:    c(N)    - Ch B, section N
;                       k(N)
;                       d(N)
;                       k(N)    (copy)
;                       c(N-1)  - Ch B, section N-1
;                       k(N-1)
;                       d(N-1)
;                       k(N-1)  (copy)
;                       ...           
...
;                       k(1)    (copy)
;
;                       v(1)  - Ch B, moving average part
;                       v(2)
;                       ...
;                       v(N+1)
;
;                       ... (coef and data grow towards each other)
;                       state(N-1)  - Ch B, section N-1
;                       state(N)    - Ch B, section N
; 37fh  _data_ptr_b:    state(N+1)  - Ch B, memory location for allpass output
;
;
; 380h  _coef_ptr_a:    c(N)    - Ch A, section N
;                       k(N)
;                       d(N)
;                       k(N)    (copy)
;                       c(N-1)  - Ch A, section N-1
;                       k(N-1)
;                       d(N-1)
;                       k(N-1)  (copy)
;                       ...           
...
;                       k(1)    (copy)
;
;                       v(1)  - Ch A, moving average part
;                       v(2)
;                       ...
;                       v(N+1)
;
;                       ... (coef and data grow towards each other)
;                       state(N-1)  - Ch A, section N-1
;                       state(N)    - Ch A, section N
; 3ffh  _data_ptr_a:    state(N+1)  - Ch A, memory location for allpass output
;
;
; Coeficent values are stored = int[(2^15)*true_coef_value]
;                             = int[32768*true_coef_value]
;
;**********************************************************************
        .global _lattice_2_a    ; declare function as global so c-code can find it
_lattice_2_a:   ; Ch A, 2nd order filter:
    ...
    ; Output allpass part for testing: 
;       adrk    2           ; AR0+2 -> AR0 (point to state3)
;       lacl    *           ; state3 -> ACC
;       sacl    _out_a      ; ACC -> out
        
    ; Compute moving average part of filter:
        lacl    #0          ; 0 -> ACC
        lt      *+,AR2      ; state1 -> T
        mpy     *+,AR0      ; T*v1 -> P
        lta     *+,AR2      ; state2 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v2 -> P
        lta     *+,AR2      ; state3 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v3 -> P
        apac                ; ACC+P -> ACC
        sach    _out_a      ; ACC -> out
...
;**********************************************************************
        .global _lattice_2_b    ; declare function as global so c-code can find it
_lattice_2_b:   ; Ch B, 2nd order filter:

; Start of Ch B
        lacl    _in_b       ; Ch B in -> ACC
        bit     _assembly_flag, 13  ; cascade_flag -> TC
        bcnd    lattice_2_skip,NTC  ; skip cascade hold if flag not set
        lacl    _out_a      ; Ch A out -> ACC
lattice_2_skip:
        sacl    _out_b      ; ACC -> input(_out_b)
...
;**********************************************************************
        .global _lattice_4_a    ; declare function as global so c-code can find it
_lattice_4_a:   ; Ch A, 4th order filter:
    ...
    ; Forward:      
        ...
        lt      _in_a       ; _in_a -> T
        mpy     *+,AR0      ; T*c4 -> P
        sbrk    1           ; AR0-1 -> AR0 (point to state4)
        lt      *+,AR2      ; state2 -> T
        mpya    *+          ; ACC+P -> ACC, T*k4 -> P
        spac                ; ACC-P -> ACC
        sach    temp        ; ACC -> temp
    ; Backward:
        lacl    #0          ; 0 -> ACC
    ...
    ; Forward:      
        ...
        lt      temp        ; temp -> T
        mpy     *+,AR0      ; T*ci -> P
        sbrk    1           ; AR0-1 -> AR0 (point to statei)
        lt      *+,AR2      ; statei -> T
        mpya    *+          ; ACC+P -> ACC, T*ki -> P
        spac                ; ACC-P -> ACC
        sach    _out_a      ; ACC -> temp2(_out_a)
    ; Backward:
        lacl    #0          ; 0 -> ACC
        mpy     *+          ; T*di -> P
        lt      temp        ; temp -> T
        mpya    *+,AR0      ; ACC+P -> ACC, T*ki -> P
        apac                ; ACC+P -> ACC
        sach    *-,AR2      ; ACC -> state(i+1)
    ...
    ; Output allpass part for testing: 
;       adrk    4           ; AR0+4 -> AR0 (point to state5)
;       lacl    *           ; state5 -> ACC
;       sacl    _out_a      ; ACC -> out
    ...
    ; Compute moving average part of filter:
        ...
        lta     *+,AR2      ; state3 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v3 -> P
        lta     *+,AR2      ; state4 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v4 -> P
        lta     *+,AR2      ; state5 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v5 -> P
        apac                ; ACC+P -> ACC
        sach    _out_a      ; ACC -> out
...
;**********************************************************************
        .global _lattice_4_b    ; declare function as global so c-code can find it
_lattice_4_b:   ; Ch B, 4th order filter:

; Start of Ch B
        lacl    _in_b       ; Ch B in -> ACC
        bit     _assembly_flag, 13  ; cascade_flag -> TC
        bcnd    lattice_4_skip,NTC  ; skip cascade hold if flag not set
        lacl    _out_a      ; Ch A out -> ACC
lattice_4_skip:
        sacl    _out_b      ; ACC -> input(_out_b)
...
;**********************************************************************
        .global _lattice_8_a    ; declare function as global so c-code can find it
_lattice_8_a:   ; Ch A, 8th order filter:
    ...
    ; Forward:      
        ...
        lt      _in_a       ; _in_a -> T
        mpy     *+,AR0      ; T*c8 -> P
        sbrk    1           ; AR0-1 -> AR0 (point to state8)
        lt      *+,AR2      ; state8 -> T
        mpya    *+          ; ACC+P -> ACC, T*k8 -> P
        spac                ; ACC-P -> ACC
        sach    temp        ; ACC -> temp
    ; Backward:
        lacl    #0          ; 0 -> ACC
    ...
    ; Output allpass part for testing: 
;       adrk    8           ; AR0+8 -> AR0 (point to state9)
;       lacl    *           ; state9 -> ACC
;       sacl    _out_a      ; ACC -> out
    ...
    ; Compute moving average part of filter:
        ...
        lta     *+,AR2      ; state5 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v5 -> P
        lta     *+,AR2      ; state6 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v6 -> P
        lta     *+,AR2      ; state7 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v7 -> P
        lta     *+,AR2      ; state8 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v8 -> P
        lta     *+,AR2      ; state9 -> T, ACC+P -> ACC
        mpy     *+,AR0      ; T*v9 -> P
        apac                ; ACC+P -> ACC
        sach    _out_a      ; ACC -> out
...
;**********************************************************************
        .global _lattice_8_b    ; declare function as global so c-code can find it
_lattice_8_b:   ; Ch B, 8th order filter:

; Start of Ch B
        lacl    _in_b       ; Ch B in -> ACC
        bit     _assembly_flag, 13  ; cascade_flag -> TC
        bcnd    lattice_8_skip,NTC  ; skip cascade hold if flag not set
        lacl    _out_a      ; Ch A out -> ACC
lattice_8_skip:
        sacl    _out_b      ; ACC -> input(_out_b)
